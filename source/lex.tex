...
\indextext{alternative preprocessing token|see{preprocessing token, alternative}}
\indextext{digraph|see{preprocessing token, alternative}}
...
\item
Each alternative preprocessing token (\tref{lex.digraph}) is replaced by its
corresponding primary token, then  each preprocessing token is converted into a
token\iref{lex.token}.
...
itself and not as the first character of the alternative preprocessing token \tcode{<:}.
...
\indextext{preprocessing token!alternative|(}%
\begin{bnf}
...
preprocessing tokens that are used in the syntax of the preprocessor.
Each \grammarterm{operator-or-punctuator} is converted to a single \grammarterm{token}
in translation phase 7\iref{lex.phases}.%

\pnum
Alternative preprocessing token representations are provided for some operators and
punctuators.
\begin{footnote}
\indextext{digraph}%
...
characters. Nonetheless, those alternative preprocessing tokens that aren't lexical
keywords are colloquially known as ``digraphs''.
\end{footnote}
In all respects of the language, each alternative preprocessing token behaves the
...
spelling, but the \grammarterm{preprocessing-token}s can otherwise be freely interchanged.
\end{footnote}
The set of alternative preprocessing tokens is defined in
\tref{lex.digraph}.

\begin{tokentable}{Alternative preprocessing tokens}{lex.digraph}{Alternative}{Primary}
\tcode{<\%}             &   \tcode{\{}         &
\keyword{and}           &   \tcode{\&\&}       &
...
literals, and alternative preprocessing tokens containing alphabetic characters.
\end{note}
\indextext{token|)}
...
\pnum
\begin{note}
Several alternative preprocessing tokens (\tref{lex.digraph}) that are
lexically equivalent to an identifier are replaced by their primary token at
the start of translation phase 7, so are not available as identifers or
keywords.
\end{note}
